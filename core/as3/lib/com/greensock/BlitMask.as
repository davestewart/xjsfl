 ...
 **/
	public class BlitMask extends Sprite {
		/** @private **/
		public static var version:Number = 0.21;
		...
		protected function _render():void {
			...
			if (x <= _bounds.x) {
				...
				if (!_smoothing) {
					_destPoint.x += 1; //necessary compensation (not entirely sure why yet)
				}
				_sliceRect.x = 0;
			} else {
				_destPoint.x = 0;
			...
			_sliceRect.x += xOffset - 1; //subtract 1 to make sure the whole image gets included - without this, a very slight vibration can occur on the edge during animation.
			...
			_tempMatrix.tx = xOffset - 1; //subtract 1 to compensate for the pixel we added above where we created the offset values.
...
//---- GETTERS / SETTERS --------------------------------------------------------------------
		...
		public function set bitmapMode(value:Boolean):void {
			if (_bitmapMode != value) {
				...
				update(null);
				_target.mask = _bitmapMode ? null : this;
