...
// methods
	 ...
	 */
	Output.print = function(content, title, output)
	{
	 ...
	 */
	Output.inspect = function(obj, label, maxDepth, print)
	{
		...
		// methods
			...
			// traversal functions
				...
				function processObject(obj)
				{
					...
					{
						//fl.trace('~' + key)
						if( ! key.match(illegal))// && obj.hasOwnProperty(i)
						{
							processLeaf(obj, key);
						}
						else
					...
					switch(type)
					{
						...
						case 'object':
							...
							*/
							...
							{
								var matches = String(value).match(/^\[object ([_\w]+)/);
								className	= matches ? matches[1] : 'Unknown';
							}
						break;
	 ...
	 */
	Output.traceHierarchy = function(rootElement)
	{
		//var values = Array.slice.call(this, arguments).join(', ')
		//fl.trace(values);
	}
